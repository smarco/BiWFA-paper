...
names(stats_df) <- c('error', 'seq.length', 'seq', 'mode', 'replicate', 'value', 'statistic', 'error.rate')
...
levels(statsWithMetadata_df$error)[match("0.4",levels(statsWithMetadata_df$error))] <- "Error rate = 40%"
...
# Plots
# Remove the error rate because can be slightly different between BiWFA and WFA-high
z <- statsWithMetadata_df[statsWithMetadata_df$statistic == 'time_ms', ] %>%
  select(-error.rate) %>%
  spread(key = mode, value = value) %>%
  mutate(BiWFA_div_WFA = log2(BiWFA/`WFA-high`))
pz <- ggplot(
  z,
  aes(x = seq.length, y = BiWFA_div_WFA, alpha=I(0.5))
) +
  geom_point() +
  facet_wrap (
    ~error,
  ...
  ggtitle('Time performance comparison between BiWFA and WFA') +
  guides(color=guide_legend(title="Algorithm")) +
  xlab("Sequence length") + ylab(expression("log"[2]("BiWFA / WFA-high"))) +  theme_bw() +
  theme(
...
pz
ggsave(plot = pz, paste0('FigureS2', '.pdf'), width = 40, height = 25, units = "cm", dpi = 300, bg = "transparent", limitsize = FALSE)
ggsave(plot = pz, paste0('FigureS2', '.png'), width = 40, height = 25, units = "cm", dpi = 300, bg = "transparent", limitsize = FALSE)
...
# Memory
px <- ggplot(
  statsWithMetadata_df[statsWithMetadata_df$statistic != 'time_ms', ],
...
py <- ggplot(
  statsWithMetadata_df[statsWithMetadata_df$statistic == 'time_ms', ],
  aes(x = seq.length, y = value, color = mode, alpha=I(0.5))
) +
  geom_point() +
  facet_wrap (
    ~error,
...
pxy
ggsave(plot = pxy, paste0('FigureSxxx', '.pdf'), width = 30, height = 40, units = "cm", dpi = 300, bg = "transparent", limitsize = FALSE)
ggsave(plot = pxy, paste0('FigureSxxx', '.png'), width = 30, height = 40, units = "cm", dpi = 300, bg = "transparent", limitsize = FALSE)
